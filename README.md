...
- Room 2.8.4 기반 기록·추억 찜 저장, 사용자 ID + Space ID로 조회 범위 분리
- DB Flow → MemoryViewModel → 지도·목록·상세·갤러리·통계에 동일 데이터 반영
- 방문 월·함께한 사람·태그·검색어·추억 찜 복합 필터와 초기화
- 저장 시 사진을 앱 전용 폴더에 복사, 원본 삭제 후에도 사본 유지
- 사진 최대 8장·한 장당 20MB, 실패한 복사 정리, 동일 기록 ID 재시도 시 중복 방지
- 로딩·읽기 오류·재시도, 저장 성공 후 상세 이동, 찜 저장 실패 안내
- Room 스키마 버전 1을 `app/schemas`에 내보내며 파괴적 마이그레이션은 사용하지 않음

현재 저장은 **이 기기 안에서만 유지**된다. 앱 삭제·로컬 데이터 초기화 시 기록과 사진 사본도
사라지며 서버 백업·기기 간 동기화는 아직 없다. 저장 전 임시 작성 내용의 영구 보관도 아직 없다.
기존 미리보기 세션은 자동 이관하지 않으며, 5주차 실행 앱의 `기록 저장`으로 새로 저장한 기록부터 보관한다.
로컬 초기화는 기록 DB → 사진 사본 → 온보딩 정보 순으로 실행된다. 두 저장소를 아우르는
단일 트랜잭션은 아니므로 중간 실패 시 일부 데이터가 삭제될 수 있고, 안내 후 다시 초기화할 수 있다.
로그아웃은 데이터를 지우지 않는다. 가보고 싶은 장소 찜은 여전히 미리보기 상태이며 추억 찜과 구분한다.

### 유지되는 4주차 기록 작성·사진 선택
...
작성 중에는 선택한 URI를 참조하고 저장 시 사본을 만든다. 저장 전에 사진 접근 권한이 해제되거나
원본이 삭제되면 재선택해야 한다. 시스템 선택창 취소 시 기존 선택은 유지된다.
...
프로필·Space·저장한 추억·사진은 앱을 종료해도 유지된다. 온보딩 파일과 기록 DB는 자동 백업·기기 이전
대상에서 제외하며 사진 사본은 `noBackupFilesDir/memory-photos`에 저장한다.
...
3. **첫 기록**: 중앙 + 또는 첫 장소 기록하기 → 장소·날짜·메모 → 앨범에서 사진 선택(선택 사항) → 기록 저장 → 기록 상세.
4. **다시 보기**: 지도 마커 → 장소 상세 → 기록 상세 → 사진 갤러리. 기록 탭에서도 같은 기록을 조회한다.
5. **다음 장소**: 우리 → 가보고 싶은 곳 → 샘플 장소 추가 또는 해제.
6. **복원**: 앱을 종료한 뒤 재실행하면 프로필·Space·기록·사진·추억 찜이 남고 우리 화면으로 이동한다.
...
| domain/memory | 기록 Repository 계약, 소유 범위, 날짜 검증·필터 규칙 |
| data/local | DataStore·Room Entity/DAO·사진 사본 관리·Repository·Hilt 바인딩 |
| feature/map, records, us, my | 메인 화면과 가보고 싶은 곳 |
| feature/memory | 기록 ViewModel·저장소 연결, 작성, 장소 선택, 상세, 사진 갤러리 |
...
실행 앱의 추억은 Room에서 관찰한 목록으로 투영한다. DB 기록 전체를 SavedState에 중복 보관하지 않는다.
장소 찜과 기존 Preview는 OurMapAppContent의 미리보기 상태에 남긴다.
...
중단된 처리 상태는 초기화해 다시 저장할 수 있다. 실행 앱은 Repository 저장과 DB 스냅샷 조회가
끝난 뒤 상세 화면으로 이동한다. 동일 ID가 이미 저장됐다면 기존 기록을 돌려줘 재시도를 안전하게 한다.
사진 복사 중단 시 미완성 사본을 정리하고, 강제 종료로 남은 미참조 파일은 다음 저장 시 정리한다.
사진을 복사한 뒤 짧은 DB insert 구간만 취소 불가로 실행해 커밋된 기록의 사진을 잘못 삭제하지 않는다.
Home이 Composition에 있는 동안 Flow를 관찰하며 읽기 실패 시 기존 화면 데이터와 입력을 유지하고 재시도한다.
목록 필터는 로컬 스냅샷에 적용한다. SQL 검색·Paging과 대량 기록 성능 최적화는 후속 범위다.
...
5주차 검증(2026-09-07): Debug 빌드, 단위 테스트 24개, 계측 테스트 17개 통과.
전체 계측 테스트를 글씨 2배·다크 모드와 기본 글씨·밝은 모드에서 각각 실행했다.
Room 파일 재열기, 사용자·Space별 격리, 동일 ID 재시도, 찜 저장, 원본 삭제 후 사진 사본 디코딩,
복사 실패 정리, 초기화 시 원본 보존, 실제 Room 홈에서 저장→상세→찜→검색·필터 초기화를 검증했다.
처음 실패한 프로필·찜 갱신은 NavEntry가 최신 관찰 상태를 읽도록 수정했고,
큰 글씨에서의 스크롤·검색 키보드 동작도 보완 후 재검증했다.
실제 MainActivity(Hilt 진입)에서 테스트 프로필·Space와 메모를 만들고 강제 종료·재실행한 뒤
기록 수와 메모 내용 복원도 확인했다. Lint 오류 0개, 경고 25개(업데이트·템플릿 리소스).
실물 기기·원격 CI·시스템 Photo Picker 직접 조작은 이번 주차 검증 범위에 포함하지 않았다.
...
5주차에 Room Runtime·KTX·KSP compiler 2.8.4와 계측 테스트용 코루틴 테스트를 추가했다.
인증 SDK·지도 SDK는 해당 주차에 추가한다.
...
- [Room 저장소](https://developer.android.com/training/data-storage/room)
